# Pico W Firmware — Retro Voice Agent

Arduino sketch for the Raspberry Pi Pico W on the 52Pi Breadboard Kit Plus.
Push-to-talk voice capture → live WSS stream → green-phosphor TFT answer.
...
to `pico_voice_agent/secrets.h` and fill in WiFi + backend host + the shared
`PICO_AUTH_TOKEN` (must equal the backend's). `secrets.h` is gitignored.

## Bring-up order (matches brief §7 — do NOT skip steps)
...
2. **Validate the mic in isolation (critical).** The SPH0645 has a 1-bit clock
   ...
   - The PIO program encodings in `pico_i2s.pio.h` were machine-verified; the
     analog/level tuning above is the expected place to iterate.

3. **Validate TLS in isolation (second risk).** With `USE_CA_PINNING 0`
   (default, in `ca_cert.h`) confirm the WSS handshake to `wss://<app>.fly.dev`
   succeeds and `{"type":"ready"}` comes back (link reports OK at boot). Only
   then, if you want an authenticated link, set `USE_CA_PINNING 1` and paste the
   ISRG Root X1 PEM (see `ca_cert.h`). Test on a trusted network first — TLS
...
4. **Full push-to-talk flow.** Remove the debug dump. Press Button 1 to record,
   ...
   seconds. The `WiFiClientSecure`/socket is opened once and kept alive across
   turns (memory rule, brief §6).
...
| `secrets.h` | your keys (gitignored; copy from `.example`) |
| `pico_i2s.pio.h` | hand-assembled + machine-verified PIO I2S program |
